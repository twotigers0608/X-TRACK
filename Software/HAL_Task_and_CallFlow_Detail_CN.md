...
## 10. 电源模块（Power）设计思路与代码执行过程（详细）

> 本节从“设计目标 -> 状态变量 -> 关键函数 -> 全链路时序 -> 与上层协作”五个角度，深入解释 `HAL_Power.cpp`。

### 10.1 设计目标（为什么这样实现）

Power 模块不是只做“读电量”，而是统一处理四类能力：

1. **供电保持/断电控制**：通过 `CONFIG_POWER_EN_PIN` 控制系统上电保持与断电；
2. **电池采样与电量估算**：ADC 采样电压，映射为 mV 和百分比；
3. **自动低功耗关机策略**：按“无操作超时”触发关机请求；
4. **关机前收尾回调**：先回调上层保存数据，再真正拉低电源脚下电。

因此它实际上是“**电源状态机 + 关机仲裁器**”。

### 10.2 内部状态变量（Power 全局结构）

`Power` 结构中的关键字段：

- `LastHandleTime`：最后一次“活跃时间”戳（用于超时判断）；
- `AutoLowPowerTimeout`：自动关机超时秒数；
- `AutoLowPowerEnable`：自动低功耗开关；
- `ShutdownReq`：关机请求标记（请求与执行解耦）；
- `ADCValue`：最近一次电池 ADC 原始值；
- `EventCallback`：关机前回调（在 `setup()` 中被绑定为 `App_Uninit`）。

这个状态设计的重点是：**请求关机不立即下电，而是先置位 `ShutdownReq`，再由监控任务统一执行**。
...
- `Power_Init()`：
  1. 清零状态并设置默认超时；
  2. 控制 `POWER_EN` 引脚完成上电保持时序（等待后拉高）；
  3. 初始化 ADC 采样通道与充电检测脚；
  4. 默认关闭自动低功耗（`Power_SetAutoLowPowerEnable(false)`）。

- `Power_ADC_Init()`：完成 ADC 时钟、模式、校准配置。

#### B) 采样与计算层

- `Power_ADC_Update()`：采用“两拍法”更新 ADC：
  - 第 1 拍：配置通道并启动转换；
  - 第 2 拍：读取上次转换结果写入 `Power.ADCValue`。

- `Power_GetInfo()`：将 `ADCValue` 转换为：
  - `voltage`（mV，带分压倍数与上下限钳制）；
  - `usage`（0~100%，按最小/满充电压线性映射）；
  - `isCharging`（由充电检测脚给出）。

#### C) 策略层

- `Power_SetAutoLowPowerEnable(en)`：开关策略并刷新 `LastHandleTime`；
- `Power_SetAutoLowPowerTimeout(sec)`：设置超时阈值；
- `Power_HandleTimeUpdate()`：有用户操作/业务活跃时更新心跳时间戳。

#### D) 关机执行层

- `Power_Shutdown()`：仅置位请求（`CM_EXECUTE_ONCE` 保证一次性）；
- `Power_EventMonitor()`：真正执行下电：
  1. 若有 `EventCallback` 先调用（保存配置/轨迹等）；
  2. 背光渐暗；
  3. 拉低 `POWER_EN` 下电；
  4. 清除 `ShutdownReq`。
...
- **10ms IRQ 回调路径**：调用 `Power_Update()`；
- **100ms 任务路径**：调用 `Power_EventMonitor()`。

这两条路径组合成“软状态机”：

```mermaid
flowchart TD
    IRQ[10ms 中断] --> U[Power_Update]
    U --> A[每 1000ms 执行 Power_ADC_Update]
    U --> B{自动低功耗使能?}
    B -- 否 --> END1[返回]
    B -- 是 --> C{是否超时?}
    C -- 否 --> END1
    C -- 是 --> D[Power_Shutdown 置位 ShutdownReq]

    TM[100ms 任务] --> M[Power_EventMonitor]
    M --> E{ShutdownReq?}
    E -- 否 --> END2[返回]
    E -- 是 --> F[EventCallback(App_Uninit)]
    F --> G[Backlight_SetGradual(0)]
    G --> H[digitalWrite POWER_EN LOW]
    H --> I[ShutdownReq=false]
```

这样做的工程收益：

- 中断路径只做轻量逻辑（判断/置位），避免中断里做重操作；
- 真正下电在任务上下文执行，便于串行完成“保存 -> 灭屏 -> 断电”。

### 10.5 与上层 App/DataProc 的协同

#### 1) App 协同（关机保存）

`setup()` 会调用 `HAL::Power_SetEventCallback(App_Uninit)`，使得关机时先执行：
...
编码器长按事件会调用 `Power_Shutdown()`，形成“用户触发关机请求 -> Power 统一执行关机”的流程。

#### 3) 状态协同（电量展示）

DataProc/页面可通过 `Power_GetInfo()` 获取电压/电量/充电状态，用于状态栏图标与系统信息页显示。

### 10.6 代码阅读建议（按入口跟踪）

建议按以下顺序阅读源码，最容易建立完整心智模型：

1. `Power_Init`（看硬件与默认策略）
2. `Power_Update`（看定时采样与超时判定）
3. `Power_Shutdown`（看请求置位）
4. `Power_EventMonitor`（看真正下电链路）
5. `Power_GetInfo`（看数据对外输出）

### 10.7 可优化点（可选）

1. 电量映射目前线性，后续可改为电池放电曲线查表；
2. `Power_ADC_Update` 两拍采样已足够轻量，如需更稳定可加滑动滤波；
3. 下电前可增加“保存完成 ACK/超时保护”日志，提升诊断性。
